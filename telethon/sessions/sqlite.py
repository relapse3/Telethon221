...
class SQLiteSession(MemorySession):
    ...
    def __init__(self, session_id=None, store_tmp_auth_key_on_disk:bool=False):
        ...
        if c.fetchone():
            ...
            if tuple_:
                if len(tuple_) == 5:
                    self._dc_id, self._server_address, self._port, key, self._takeout_id = tuple_
                    self._with_tmp_key = False
                elif len(tuple_) == 6:
                    self._dc_id, self._server_address, self._port, key, self._takeout_id, _ = tuple_
                    self._with_tmp_key = True
